...
public class CouplingFromThePast {
	...
	// Lancement de l'Echantillionnage
	public void runIndependentTrials(ResultDataStructure data,
		...
		try {
			...
			for (int i = 0; i < numberOfSamples; i++) {
				...
				//System.out.println(this.distribution.getClass());
				//System.out.println(this.distribution.getNextStateProposer().getClass());
				//System.out.println(this.algorithm.getClass());
				if (stop) {
					return;
				}

				OrderedBaseObject baseObject = runOnce().getBaseObject();
				System.out.println("Ensemble :"+baseObject.toString());

				if (stop) {
					return;
				}
				((TwoStepSamplingDistribution) this.distribution).drawSubSet(baseObject, data);
				pp.addStepsAndPrint(1);
				Thread.sleep(0);
			}
			pp.printCompletion();
